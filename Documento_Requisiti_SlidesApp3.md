...
> Questo documento descrive i requisiti per il progetto **SlidesApp**, un'applicazione web per la creazione e la gestione di presentazioni digitali, sviluppata nell'ambito del modulo `03_Sviluppo_Web_e_Database`.
...
Il diagramma dei casi d'uso è rappresentato nel file `docs/use_cases.png` o nel sorgente `docs/use_cases.puml`.
...
> Questo ER è utile per la fase di progettazione. La relazione `USER → PRESENTATIONS` è uno-a-molti: ogni presentazione appartiene a un utente. La relazione `PRESENTATIONS → SLIDES` è uno-a-molti con cascade delete. La tabella `OTP_TOKENS` memorizza i codici email temporanei con scadenza.
...
> Il diagramma UML aiuta a capire le classi principali del dominio. Le classi di servizio e accesso ai dati (repository) possono essere aggiunte se si vuole rappresentare anche l'architettura applicativa, ma non sono necessarie nel diagramma delle sole entità principali.
...
- Tenere un file `README.md` con istruzioni di installazione e uso: creazione dell'ambiente virtuale, installazione dipendenze (`pip install flask flask-babel pyotp qrcode[pil] Flask-Mail`), inizializzazione del DB (`python setup_db.py`), compilazione traduzioni (`pybabel compile -d app/translations`) e avvio (`python run.py`).
- Usare `.gitignore` per escludere `__pycache__/`, `.venv/`, `instance/` e i file `*.mo` (si rigenerano da build).
- Includere i diagrammi di progetto se sono stati realizzati.
- Fare commit frequenti e significativi lungo tutto lo sviluppo.
